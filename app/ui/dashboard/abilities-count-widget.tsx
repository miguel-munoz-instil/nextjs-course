...
import { fetchAbilitiesCount } from '@/app/lib/data';

export default async function AbilitiesCountWidget() {
  const { collected, total } = await fetchAbilitiesCount();
  ...
  return (
    <div className="rounded-xl bg-void-black border-2 border-thorny-purple p-6 shadow-sm hover:border-needle-green transition-colors">
      <div className="flex items-center justify-between">
        ...
        <div className="flex h-12 w-12 items-center justify-center rounded-full bg-thorny-purple/20">
          <BoltIcon className="h-6 w-6 text-thorny-purple" />
        </div>
      </div>
