...
import { updateNeedleUpgrade, NeedleUpgradeState } from '@/app/lib/actions';
import { useActionState } from 'react';
import { NeedleUpgradeForm } from '@/app/lib/definitions';

export default function EditNeedleUpgradeForm({ upgrade }: { upgrade: NeedleUpgradeForm }) {
  const initialState: NeedleUpgradeState = { message: null, errors: {} };
  const updateNeedleUpgradeWithId = updateNeedleUpgrade.bind(null, upgrade.id);
  const [state, formAction] = useActionState(updateNeedleUpgradeWithId, initialState);
  ...
  return (
    <form action={formAction}>
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            <input
              ...
              name="percentage"
              type="number"
              defaultValue={upgrade.percentage}
              min={1}
              className="peer block w-full rounded-md border border-thorny-purple bg-black py-2 px-3 text-sm text-silk-white placeholder:text-silk-silver focus:border-thorny-red focus:outline-none focus:ring-1 focus:ring-thorny-red"
            ...
            <div className="flex gap-4">
              <div className="flex items-center">
                <input
                  ...
                  type="radio"
                  value="pending"
                  defaultChecked={upgrade.status === 'pending'}
              ...
              <div className="flex items-center">
                <input
                  ...
                  type="radio"
                  value="collected"
                  defaultChecked={upgrade.status === 'collected'}
      ...
      <div className="mt-6 flex justify-end gap-4">
        ...
        <Button type="submit">Update Needle Upgrade</Button>
      </div>
    </form>
  );
}
