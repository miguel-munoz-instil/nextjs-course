import { UpdateSilkHeart, DeleteSilkHeart } from '@/app/ui/silk-hearts/buttons';
import SilkHeartStatus from '@/app/ui/silk-hearts/status';
import { fetchFilteredSilkHearts } from '@/app/lib/data';

export default async function SilkHeartsTable({
  query,
  currentPage,
}: {
  query: string;
  ...
  const hearts = await fetchFilteredSilkHearts(query, currentPage);
                ...
                <div className="flex items-center justify-between border-b border-silk-silver/30 pb-4">
                  <div>
                    <div className="mb-2 flex items-center">
                      <p className="text-silk-white">{heart.name}</p>
                    </div>
                    <p className="text-sm text-silk-silver">{heart.notes}</p>
                  </div>
                  <SilkHeartStatus status={heart.status} />
                </div>
                <div className="flex w-full items-center justify-between pt-4">
                  <div className="flex justify-end gap-2">
                    <UpdateSilkHeart id={heart.id} />
                    <DeleteSilkHeart id={heart.id} />
                  </div>
                </div>
              </div>
            ))}
