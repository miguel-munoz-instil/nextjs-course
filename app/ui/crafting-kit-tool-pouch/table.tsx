import { UpdateCraftingKitToolPouch, DeleteCraftingKitToolPouch } from '@/app/ui/crafting-kit-tool-pouch/buttons';
import CraftingKitToolPouchStatus from '@/app/ui/crafting-kit-tool-pouch/status';
import { fetchFilteredCraftingKitToolPouch } from '@/app/lib/data';

export default async function CraftingKitToolPouchTable({
  query,
  currentPage,
}: {
  query: string;
  ...
  const items = await fetchFilteredCraftingKitToolPouch(query, currentPage);
                ...
                <div className="flex items-center justify-between border-b border-silk-silver/30 pb-4">
                  <div>
                    <div className="mb-2 flex items-center">
                      <p className="text-silk-white">{item.name}</p>
                    </div>
                    <p className="text-sm text-silk-silver">{item.notes}</p>
                  </div>
                  <CraftingKitToolPouchStatus status={item.status} />
                </div>
                <div className="flex w-full items-center justify-between pt-4">
                  <div className="flex justify-end gap-2">
                    <UpdateCraftingKitToolPouch id={item.id} />
                    <DeleteCraftingKitToolPouch id={item.id} />
                  </div>
                </div>
              </div>
            ))}
