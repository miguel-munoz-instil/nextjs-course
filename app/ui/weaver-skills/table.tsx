import { UpdateWeaverSkill, DeleteWeaverSkill } from '@/app/ui/weaver-skills/buttons';
import WeaverSkillStatus from '@/app/ui/weaver-skills/status';
import { fetchFilteredWeaverSkills } from '@/app/lib/data';

export default async function WeaverSkillsTable({
  query,
  currentPage,
}: {
  query: string;
  ...
  const skills = await fetchFilteredWeaverSkills(query, currentPage);
                ...
                <div className="flex items-center justify-between border-b border-silk-silver/30 pb-4">
                  <div>
                    <div className="mb-2 flex items-center">
                      <p className="text-silk-white font-medium">{skill.name}</p>
                    </div>
                    <p className="text-sm text-silk-silver">{skill.notes || 'No notes'}</p>
                  </div>
                  <WeaverSkillStatus status={skill.status} />
                </div>
                <div className="flex w-full items-center justify-end pt-4">
                  <div className="flex justify-end gap-2">
                    <UpdateWeaverSkill id={skill.id} />
                    <DeleteWeaverSkill id={skill.id} />
                  </div>
                </div>
              </div>
            ))}
